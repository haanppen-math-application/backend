...
import com.hpmath.hpmathcoreapi.course.application.exception.InvalidCourseAccessException;
...
class CourseMapper {
    Teacher mapToTeacher(final Member member) {
        if (member.getRole().equals(Role.STUDENT)) {
            throw new InvalidCourseAccessException("학생은 반을 관리할 수 없습니다",ErrorCode.INVALID_MEMBER_TARGET);
        }
    ...
    Student mapToStudent(final Member member) {
        ...
        throw new InvalidCourseAccessException("학생만 반에 등록될 수 있습니다", ErrorCode.INVALID_MEMBER_TARGET);
    }
