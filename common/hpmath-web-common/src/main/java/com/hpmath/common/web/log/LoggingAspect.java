...
class LoggingAspect {
    ...
    @AfterThrowing(value = "within(@com.hpmath.common.web.log.WarnLoggable *)", throwing = "exception")
    ...
    @AfterThrowing(value = "within(@com.hpmath.common.web.log.ErrorLoggable *)", throwing = "exception")
    public void errorLog(JoinPoint joinPoint, Exception exception) {
        Logger logger = LoggerFactory.getLogger(joinPoint.getTarget().getClass());
        logger.warn("[ EXCEPTION ] -> " + exception.getClass().getSimpleName() + " -> " + exception.getMessage());
    }

    @Pointcut("execution(* com.hpmath.hpmathcoreapi..*Repository.*(..))")
