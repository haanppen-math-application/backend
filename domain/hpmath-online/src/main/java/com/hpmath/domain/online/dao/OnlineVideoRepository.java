...
public interface OnlineVideoRepository extends JpaRepository<OnlineVideo, Long> {
    ...
    @Query("SELECT ov FROM OnlineVideo ov JOIN FETCH ov.onlineCourse WHERE ov.id = :onlineVideoId")
    Optional<OnlineVideo> loadSingleOnlineVideoWithCourseByVideoId(@Param("onlineVideoId") final Long onlineVideoId);
}
