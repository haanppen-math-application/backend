...
public interface OnlineCategoryRepository extends JpaRepository<OnlineCategory, Long> {
    ...
    @Query("SELECT new com.hpmath.academyapi.online.dto.OnlineCategoryInfo(oc.id, oc.categoryName, oc.creationTime) FROM OnlineCategory oc WHERE oc.parentCategory.id = :categoryId")
    List<OnlineCategoryInfo> queryChildCategories(@Param("categoryId") final Long categoryId);

    @Query("SELECT new com.hpmath.academyapi.online.dto.OnlineCategoryInfo(oc.id, oc.categoryName, oc.creationTime) FROM OnlineCategory oc WHERE oc.parentCategory IS null")
